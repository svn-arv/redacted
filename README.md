...
A hook that redacts secrets from Bash tool output before your AI coding assistant sees them.

When an AI tool runs a Bash command, the full output goes into conversation context. If that output contains API keys, database URLs, or tokens, they end up on the wire. `redacted` prevents this by scanning the output and replacing secrets before they leave your machine.
...
This registers `redacted scrub` as a PostToolUse hook. Every Bash command output gets scanned before it reaches the model. Safe to run multiple times.

| Flag       | Settings file                 | Scope        |
| ---------- | ----------------------------- | ------------ |
| *(default)* | `.claude/settings.local.json` | This project |
...
The hook reads a JSON payload from stdin, runs all patterns against stdout/stderr, and either exits silently (pass-through) or outputs a JSON block response with the redacted text. The last 4 characters of each secret are kept as a hint so you can tell which key was hit without exposing the value.
...
Then remove the `redacted` entry from the `PostToolUse` array in your settings file.
