...
func TestCorpus_Recall(t *testing.T) {
	...
	cases := []struct{ name, text string }{
		...
		{"aws_sts_key", testutil.AWSSTSKey().Value},
		{"github_user_to_server", testutil.GitHubToken("ghu_").Value},
		{"slack_refresh", testutil.SlackToken("xoxe").Value},
		{"stripe_webhook_secret", testutil.StripeWebhookSecret().Value},
		{"vendor_huggingface", testutil.AIVendorKey("huggingface").Value},
		{"vendor_groq", testutil.AIVendorKey("groq").Value},
		{"vendor_openrouter", testutil.AIVendorKey("openrouter").Value},
		{"vendor_xai", testutil.AIVendorKey("xai").Value},
		{"vendor_perplexity", testutil.AIVendorKey("perplexity").Value},
		{"vendor_tavily", testutil.AIVendorKey("tavily").Value},
		{"vendor_langsmith", testutil.AIVendorKey("langsmith").Value},
		...
		{"google_api_key_v2", testutil.GoogleAPIKeyV2().Value},
		...
		{"private_key_truncated_rsa", testutil.PrivateKeyTruncated("RSA ").Value},
		{"private_key_truncated_generic", testutil.PrivateKeyTruncated("").Value},
		{"gcp_sa_private_key", testutil.GCPServiceAccountKey(testutil.RandAlphaNum(96)).Value},
		{"bearer_opaque", "Authorization: Bearer " + testutil.RandAlphaNum(32)},
		{"bearer_lowercase_header", "authorization: bearer " + testutil.RandAlphaNum(24)},
		{"basic_auth_header", "Authorization: Basic " + testutil.RandAlphaNum(24)},
		{"proxy_authorization", "Proxy-Authorization: Bearer " + testutil.RandAlphaNum(28)},
		{"mysql_pwd", "MYSQL_PWD=" + testutil.RandAlphaNum(14)},
		{"passwd_keyword", "PASSWD=" + testutil.RandAlphaNum(14)},
		{"connection_string", "CONNECTION_STRING=" + testutil.RandAlphaNum(28)},
		{"hex_under_key_suffix", "ALGOLIA_ADMIN_KEY=" + testutil.RandHex(32)},
		{"env_secret_keyword", "SECRET_KEY=" + testutil.RandAlphaNum(24)},
		{"env_secret_colon", "AUTH_TOKEN: aB3" + testutil.RandAlphaNum(17)},
